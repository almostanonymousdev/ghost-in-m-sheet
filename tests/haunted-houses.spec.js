...
test.describe('Haunted Houses Controller', () => {
  ...
  test('applyTickEffects adds +10 lust per turn while bait is active', async () => {
    await setHuntMode(page, 2);
    await setVar(page, 'baitActive', 1);
    await setVar(page, 'baitStepsRemain', 3);
  ...
  test('applyTickEffects flags baitOrgasmPending when lust was already at 100', async () => {
    await setHuntMode(page, 2);
    await setVar(page, 'baitActive', 1);
  ...
  test('applyTickEffects does NOT flag baitOrgasmPending without bait', async () => {
  ...
  test('consumeBaitOrgasm zeros lust, drops sanity 10, leaves bait active', async () => {
    await setHuntMode(page, 2);
    await setVar(page, 'baitActive', 1);
  ...
  test('consumeBaitOrgasm is a no-op when no orgasm is pending', async () => {
  ...
  test('resetHuntFlags clears baitOrgasmPending', async () => {
    await setVar(page, 'baitOrgasmPending', 1);
  ...
  test('canBait no longer requires lust >= cost (lust is now an add)', async () => {
