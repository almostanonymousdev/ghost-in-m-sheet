...
test.describe('Witch — access and hours', () => {
  ...
  test('isDayTime is true between 10:00 and 23:00', async () => {
  ...
  test('Witch exterior shows closed message before 10 AM without a stolen key', async () => {
  ...
  test('canSneakInAtNight requires stolen key and non-daytime', async () => {
    await setVar(page, 'hours', 3);
    await page.evaluate(() => { delete SugarCube.State.variables.gotKeyFromWitch; });
  ...
  test('canStealKeyFromWitch requires corruption >= 3 and no existing key', async () => {
    await page.evaluate(() => { delete SugarCube.State.variables.gotKeyFromWitch; });
  ...
  test('WitchInsideNight passage renders cleanly for a night sneak-in', async () => {
...
test.describe('Witch — contract lifecycle', () => {
  ...
  test('canAffordContract requires money >= 35', async () => {
  ...
  test('WitchInside shows contract links during daytime when affordable', async () => {
    await setVar(page, 'hours', 12);
  ...
  test('WitchInside shows "not enough money" when broke', async () => {
    await setVar(page, 'hours', 12);
  ...
  test('WitchInside shows "already have a contract" during an active hunt', async () => {
    await setVar(page, 'hours', 12);
    await setVar(page, 'mc.money', 200);
    await setVar(page, 'firstVisitWitchShop', false);
    await setHuntMode(page, 1);
...
test.describe('Witch — side quests', () => {
  ...
  test('canOfferRescueQuest is true only when $hasQuestForRescue is undefined', async () => {
  ...
  test('rescueQuestUnlocked requires mc.lvl >= 4', async () => {
  ...
  test('canOfferCursedItemQuest requires mc.lvl >= 2 and no existing quest', async () => {
  ...
  test('cursedItemQuestActive / hasCursedItemToTurnIn reflect $gotCursedItem', async () => {
    await setVar(page, 'gotCursedItem', 0);
  ...
  test('collectCursedItemReward pays $30, clears gotCursedItem and CI flags', async () => {
    await setVar(page, 'gotCursedItem', 1);
  ...
  test('shouldAwardGwb3OnTurnIn reflects current gwb level', async () => {
  ...
  test('canOfferWeakenQuest requires mc.lvl >= 5 and no existing quest', async () => {
  ...
  test('canAskAboutMonkeyPaw requires $boughtMonkeyPawGuide === 0', async () => {
  ...
  test('canAskAboutIronclad matches wardenClothesStage 0 or 1', async () => {
  ...
  test('canAskAboutLevel3Tools is true until eventToolsOneStart is set', async () => {
...
test.describe('Witch — shop and intimate events', () => {
  ...
  test('WitchSale renders without SugarCube errors', async () => {
    await setVar(page, 'hours', 12);
    await setVar(page, 'mc.money', 500);
  ...
  test('WitchInsideMast (1-in-7 random redirect) renders cleanly', async () => {
    await setVar(page, 'hours', 12);
    await goToPassage(page, 'WitchInsideMast');
    await expectCleanPassage(page);
  });

  test('WitchTentaclesEvent renders cleanly', async () => {
    await setVar(page, 'hours', 12);
