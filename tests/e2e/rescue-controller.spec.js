...
test.describe('Missing Women — controller + church integration', () => {
  ...
  test('mustReturnToNun is true for stages 2 and 3', async ({ game: page }) => {
    await setVar(page, 'hasQuestForRescue', 2);
  ...
  test('boardPostingsOutToday is true between 18:00 and 23:59', async ({ game: page }) => {
  ...
  test('boardOnCooldown checks rescueQuest', async ({ game: page }) => {
  ...
  test('canResolveRescue requires stage < 2, correct house, and active quest', async ({ game: page }) => {
    await setVar(page, 'hasQuestForRescue', 1);
  ...
  test('canStaySubmissive requires corruption >= 6', async ({ game: page }) => {
  ...
  test('canSearchHouse requires energy >= 1', async ({ game: page }) => {
  ...
  test('rescueEventAuto is true when rescueStage is 0', async ({ game: page }) => {
    await setVar(page, 'rescueStage', 0);
  ...
  test('rescueEventRolls requires stage 1 and hours < 18', async ({ game: page }) => {
    await setVar(page, 'rescueStage', 1);
  ...
  test('possessedPassageFor returns correct passage for each girl', async ({ game: page }) => {
  ...
  test('canStartNunQuest is true when quest is 0 and Rain not met', async ({ game: page }) => {
    await setVar(page, 'hasQuestForRescue', 0);
  ...
  test('canStartNunQuest is false after Rain is met', async ({ game: page }) => {
    await setVar(page, 'hasQuestForRescue', 0);
    await setVar(page, 'relationshipWithRain', 0);
    expect(await callSetup(page, 'setup.Church.canStartNunQuest()')).toBe(false);
  });

  test('showMissingPersonsBoard requires Rain met and quest defined', async ({ game: page }) => {
  ...
  test('shouldRedirectToNunQuest is true when quest is 2 or 3', async ({ game: page }) => {
    await setVar(page, 'hasQuestForRescue', 2);
