...
test.describe('Cursed Home Items — controller', () => {
  ...
  test('isItemCursed is false when no curse is active', async () => {
  ...
  test('isItemCursed is true only for the active item', async () => {
  ...
  test('cursedItemLabel returns the human-readable label', async () => {
  ...
  test('cursedItemLabel returns empty string when nothing is cursed', async () => {
    await setVar(page, 'cursedHomeItem', '');
    expect(await callSetup(page, 'setup.CursedItems.cursedItemLabel()')).toBe('');
  });

  test('clearCurse resets both state variables', async () => {
  ...
  test('rollForCursedItem always picks one of the five items (bias the RNG)', async () => {
  ...
  test('rollForCursedItem skips the curse when the 40% roll fails', async () => {
...
test.describe('Cursed Home Items — home hub integration', () => {
  ...
  for (const { key, hub, event, label } of CURSED_ITEMS) {
    test(`${label} hub (${hub}) renders with cursed item active`, async () => {
      await primeCurse(page, key);
    ...
    test(`${label} event passage (${event}) renders cleanly`, async () => {
      await primeCurse(page, key);
  ...
  test('only one cursed item can be active at a time', async () => {
    await primeCurse(page, 'tv');
  ...
  test('Livingroom without curse shows normal Use PC link (no redirect)', async () => {
    await setVar(page, 'cursedHomeItemActive', 0);
    await setVar(page, 'cursedHomeItem', '');
