/* Chapel's meters.js v1.0.1 (with the local "M-on-window" tweak) plus
 * the trivial-accessor helper used by every controller IIFE. Both load
 * here at the root of the passages tree so they're available before
 * the per-domain controllers under any subdirectory run. */
(function () {
    'use strict';
...
/* For a $-var that holds an enum stage value, splice mechanical per-
 * stage `is<X>()` predicates and/or `mark<X>()` mutators directly onto
 * the controller `api` object. `spec.is` / `spec.mark` are
 * `{ methodName: STAGE_KEY }` pairs, where STAGE_KEY indexes into
 * `stages`. Lets controllers keep callsite-friendly method names while
 * collapsing the body wall:
 *
 *   setup.defineStageAccessors(api, sv, 'hasQuestForRescue',
 *       setup.RescueQuestState, {
 *           is:   { isQuestAvailable: 'AVAILABLE', hasActiveQuest: 'ACTIVE' },
 *           mark: { markQuestFailed: 'FAILED' }
 *       });
 */
setup.defineStageAccessors = function (api, host, key, stages, spec) {
    Object.keys(spec.is || {}).forEach(function (methodName) {
        var val = stages[spec.is[methodName]];
        api[methodName] = function () { return host()[key] === val; };
    });
    Object.keys(spec.mark || {}).forEach(function (methodName) {
        var val = stages[spec.mark[methodName]];
        api[methodName] = function () { host()[key] = val; };
    });
};

/* Daily-cooldown registry. Each owning controller registers its CDs at
 * module load (setup.Cooldowns.registerDaily('witchNight')); midnight
 * rollover (setup.Tick.resetCooldowns) calls resetDaily() to zero them
 * all at once. Replaces the hardcoded perDayZero list that lived in
 * TickController and duplicated each controller's OWNED_VARS.
 *
 * Semantic for binary cooldowns: "available" means undefined / 0;
 * anything else (typically 1) means "on cooldown". Controllers that
 * want a different shape (multi-stage counters like exorcism or
 * succubusEvent.eventCD) keep their own logic — the registry only owns
...
setup.Cooldowns = (function () {
    ...
    return {
        ...
        registerDaily: function (name) {
            if (DAILY.indexOf(name) === -1) DAILY.push(name);
        },
        /* Test/inspection hook — returns a copy. */
        listDaily: function () { return DAILY.slice(); },
        /* True when `name` is undefined or 0. */
        available:  function (name) {
            var v = sv()[name];
            return v === undefined || v === 0;
        },
        onCooldown: function (name) { return !this.available(name); },
        start: function (name) { sv()[name] = 1; },
        clear: function (name) { sv()[name] = 0; },
        /* Zero every registered daily cooldown. Called from
           setup.Tick.resetCooldowns at midnight. */
        resetDaily: function () {
            var s = sv();
            for (var i = 0; i < DAILY.length; i++) { s[DAILY[i]] = 0; }
        }
    };
})();
